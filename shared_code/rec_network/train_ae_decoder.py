...
import sys
sys.path.append("/home/ubuntu/hdd1/yyk/true_yyk/diffsion_few_shot_2/seg_network")
...
def train_on_device(args):
    ...
    config = OmegaConf.load("/home/ubuntu/hdd1/yyk/true_yyk/diffsion_few_shot_2/configs/mvtec.yaml")
    ...
    # model kl decoder
    for param in model.module.first_stage_model.parameters():
        param.requires_grad = True
    print("this is ok !!!!!!!")
    
    optimizer = torch.optim.Adam([{"params": model.module.first_stage_model.parameters(), "lr": args.lr*gpu_num}])
    ...
    for epoch in range(args.epochs):
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            cond = {}
            cond["c_crossattn"] = [model.module.get_learned_conditioning(cond_all["c_crossattn"])]
            cond["c_concat"] =  [model.module.encode_first_stage((cond_all["c_concat"])).mode()]
            c = cond["c_concat"][0]
            shape = c.shape[1:]
            noise = torch.randn_like(c)
            t = torch.randint(400, 500, (c.shape[0],), device=device).long()
            c_noisy = model.module.q_sample(x_start=c, t=t, noise=noise)
            ...
            samples_ddim, _ = sampler.sample(S=50,
                                             ...
                                             shape=shape,
                                             verbose=False)
            gray_rec = model.module.decode_first_stage(samples_ddim)

            l2_loss = loss_l2(gray_rec,gray_batch)
            ssim_loss = loss_ssim(gray_rec, gray_batch)
            # 确保损失具有梯度
            if not l2_loss.requires_grad:
                l2_loss = l2_loss.clone().detach().requires_grad_(True)
                print("this is ok too")
            if not ssim_loss.requires_grad:
                ssim_loss = ssim_loss.clone().detach().requires_grad_(True)
            ...
            if not ddp or (ddp and dist.get_rank() == 0):
                if args.visualize and n_iter % 200 == 0:
                    visualizer.plot_loss(loss, n_iter, loss_name='all_loss')
                    
                sample_path = os.path.join(args.log_path, "samples")
                os.makedirs(sample_path, exist_ok=True)
                if args.visualize and n_iter % 100 == 0:
                    sample_augmented = aug_gray_batch.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255

                    sample_recon_target = gray_batch.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255
                    sample_recon_out = gray_rec.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255

                    h, w, _ = sample_augmented.shape
                    sample_recon_target = cv2.resize(sample_recon_target, (w, h))
                    sample_recon_out = cv2.resize(sample_recon_out, (w, h))

                    concatenated_image = np.concatenate((sample_augmented, sample_recon_target, sample_recon_out), axis=1)

                    outpath_concatenated = os.path.join(sample_path, 'concatenated_' + str(n_iter) + '.jpg')
                    cv2.imwrite(outpath_concatenated, concatenated_image)

            n_iter += 1
        scheduler.step()

        if not ddp or (ddp and dist.get_rank() == 0):
            torch.save(model.module.first_stage_model.state_dict(), os.path.join(args.checkpoint_path, run_name + "kl_refine.ckpt"))
