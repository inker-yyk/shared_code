...
from data_loader import MVTecDRAEMTestDataset_v1
...
def test(obj_name, mvtec_path, checkpoint_path, base_model_name, ldm_path, txt_path, output_path):
    ...
    model_seg.cuda(device)
    model_seg.eval()

    dataset = MVTecDRAEMTestDataset_v1(mvtec_path, obj_name, resize_shape=[img_dim, img_dim])
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        feature_list = []
        for img in cond_all["c_crossattn"]:
            img = img.to(device)
            feature = model.encode_first_stage(img).mode()
            feature_list.append(feature)
        
        cond = {}
        cond["c_crossattn"] = [torch.mean(torch.cat(feature_list, dim=0), dim=0).unsqueeze(dim=0)]
